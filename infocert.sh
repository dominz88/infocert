...
function download_certificate {
    echo | openssl s_client -servername "${DOMAIN}" -connect "${IP}:${PORT}" 2>/dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p'
}

function get_certificate_info {
    openssl x509 -text -noout -in "${CERTIFICATE}" | grep -e "Subject:" -e "Not Before" -e "Not After" -e "DNS:" | awk '{$1=$1};1'
}
